...
export default function CartProvider({children}: CartProviderProps) {
  ...
  const addToCart = async (product: ProductProps) => {
    setLoadingCart(true);
    const indexItem = await cart.findIndex(
      (item: ProductCartProps) => item.product.id === product.id,
    );
    ...
    if (indexItem !== -1) {
      ...
      cartList[indexItem].total =
        cartList[indexItem].amount * Number(cartList[indexItem].product.price);
  ...
  const removeToCart = async (product: ProductProps) => {
    ...
    if (cart[indexItem]?.amount > 1) {
      ...
      cartList[indexItem].amount = cartList[indexItem].amount - 1;
      cartList[indexItem].total =
        cartList[indexItem].total - cartList[indexItem].product.price;
    ...
    const removeItem = await cart.filter(
      (item: ProductCartProps) => item.product.id !== product.id,
    );
    setCart(removeItem);
